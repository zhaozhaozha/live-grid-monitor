...
**Q：关掉浏览器页面后，还会继续记录吗？**
会。v1.0.2 起广告识别已搬到服务端：只要服务在跑，就会对在播房间持续抽帧识别并写入数据库，不依赖浏览器是否打开。开播/下播时间、在线人数本来就是服务端采集的。

**Q：广告识别不准怎么办？**
调 `app.env` 里的 `AD_SCORE_THRESHOLD`（默认 0.62，调低更敏感但易误报，调高更保守但易漏报）。也可以在监控页每个格子用 📢/🚫 按钮人工补标。低配机器或多路同开时，可在 `app.env` 设 `AD_SAMPLER_ENABLED=0` 关闭采样省 CPU。

**Q：怎么导出单个直播间的报表？**
报表页每个房间右侧有「导出」按钮，导出的是该直播间一份完整台账（汇总 + 场次明细 + 广告时段明细），CSV 带 BOM，Excel 双击即可打开不乱码。
